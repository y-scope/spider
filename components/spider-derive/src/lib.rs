...
/// Rust enum as a MySQL `ENUM` column. It uses each variant's identifier name as the corresponding
/// MySQL enum string value (e.g., variant `Ready` maps to the string `'Ready'`).
///
/// # Generated methods
///
...
///
/// Returns the MySQL `ENUM(...)` type declaration string for use in DDL statements. For example, an
/// enum with variants `Red`, `Green`, `Blue` produces `"ENUM('Red','Green','Blue')"`.
///
...
///   variant name.
/// * [`sqlx::Decode<sqlx::MySql>`] — Decodes the enum by matching the database string against
///   known variant names. Returns an error for unknown values.
///
/// # Limitations
///
