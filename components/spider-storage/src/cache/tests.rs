...
impl ReadyQueueConnector for MockReadyQueue {
    async fn send_task_ready(
        ...
        if let Some(txs) = &self.worker_txs {
            ...
            for &idx in &task_ids {
                let worker =
                    self.round_robin_counter.fetch_add(1, Ordering::Relaxed) as usize % num_workers;
...
/// immediately, inject failures, or add delays.
#[async_trait]
trait TaskHandler: Send + Sync {
    /// Called for each task. Returns outputs to submit on success, or an error message to fail
    /// the instance.
...
/// Default handler: immediately completes each task with 1KB outputs.
...
/// Handler where every task always fails. Used to test retry exhaustion.
...
/// Per-task timing breakdown: time spent in `create_task_instance` (pre-execution) and
/// `complete/fail_task_instance` (post-execution).
#[derive(Debug)]
#[allow(dead_code)]
struct TaskLatency {
    task_index: TaskIndex,
    /// Duration of `create_task_instance` call.
    create_duration: Duration,
    /// Duration of `complete_task_instance` or `fail_task_instance` call.
...
struct TestResult {
    ...
    /// Per-task latencies with create/complete breakdown. One entry per dispatched task.
    task_latencies: Vec<TaskLatency>,
...
impl TestResult {
    fn report(
        ...
        let mut create_ms: Vec<f64> = self
            .task_latencies
            .iter()
            .map(|l| to_ms(&l.create_duration))
            .collect();
        let mut complete_ms: Vec<f64> = self
            .task_latencies
            .iter()
            .map(|l| to_ms(&l.complete_duration))
            .collect();
        create_ms.sort_by(|a, b| a.partial_cmp(b).expect("latencies should be comparable"));
        complete_ms.sort_by(|a, b| a.partial_cmp(b).expect("latencies should be comparable"));

        let _count = self.task_latencies.len();
        let avg_create = avg_of(&create_ms);
        let avg_complete = avg_of(&complete_ms);

        eprintln!();
        eprintln!("=== {test_name} ({num_workers} workers) ===");
        eprintln!(
            "  graph_construction:           {:>10.2} ms",
            to_ms(&graph_construction_time)
        );
        eprintln!(
            "  build_job:                   {:>10.2} ms",
            to_ms(&build_job_time)
        );
        eprintln!(
            "  total_execution:             {:>10.2} ms",
            to_ms(&self.total_execution_time)
        );
        eprintln!(
            "  tasks_dispatched:             {:>10}",
            self.tasks_dispatched
        );
        eprintln!("  --- create_task_instance ---");
        eprintln!("    avg:                         {avg_create:>10.3} ms");
        eprintln!(
            "    p50:                         {:>10.3} ms",
        ...
        eprintln!(
            "    p95:                         {:>10.3} ms",
            percentile(&create_ms, 95.0)
        );
        eprintln!(
            "    p99:                         {:>10.3} ms",
            percentile(&create_ms, 99.0)
        );
        eprintln!("  --- complete/fail_task_instance ---");
        eprintln!("    avg:                         {avg_complete:>10.3} ms");
        eprintln!(
            "    p50:                         {:>10.3} ms",
            percentile(&complete_ms, 50.0)
        );
        eprintln!(
            "    p95:                         {:>10.3} ms",
            percentile(&complete_ms, 95.0)
        );
        eprintln!(
            "    p99:                         {:>10.3} ms",
            percentile(&complete_ms, 99.0)
        );
        eprintln!(
...
async fn run_scheduled_test(
    ...
    for mut rx in worker_rxs {
        ...
        worker_handles.push(tokio::spawn(async move {
            loop {
                ...
                let task_idx = if let Ok(idx) = rx.try_recv() {
                    idx
                } else {
                ...
                let (terminal, lat) = execute_single_instance(&jcb, &*handler, task_idx).await;
    ...
    let total_tasks_reported_ready: usize =
        ready_queue_snapshot.iter().map(|(_, ids)| ids.len()).sum();
    drop(ready_queue_snapshot);

    let task_latencies = Arc::try_unwrap(latencies)
...
/// Returns `(is_terminal, Option<TaskLatency>)`. The latency is `None` if registration was
/// rejected (e.g. task already terminated by another worker).
async fn execute_single_instance(
    jcb: &Arc<JcbType>,
    handler: &dyn TaskHandler,
    task_idx: TaskIndex,
) -> (bool, Option<TaskLatency>) {
    // Time the create_task_instance call.
    ...
    // Handler decides outputs or failure (not timed — simulates external execution).
    let outcome = handler.handle_task(task_idx, &ctx).await;

    // Time the complete/fail call.
        ...
        Ok(outputs) => {
                ...
                .await
            {
                ...
                Err(e) => {
                    assert!(
                        is_rejection(&e),
                        "complete_task_instance for task {task_idx} returned unexpected error: \
                         {e:?}"
                    );
                    false
                }
...
fn build_neural_net_graph(
    ...
    build_neural_net_graph_with_policy(num_layers, width, fan_in, ExecutionPolicy::default())
}
...
/// # Metrics captured
///
/// Graph construction, `build_job`, total execution (128 workers), per-task latency
/// (avg/p50/p95/p99).
///
...
async fn test_scale_10k_independent() {
    ...
    assert_eq!(
        result.tasks_dispatched, NUM_TASKS,
...
async fn test_neural_net_ready_queue_correctness() {
    ...
    assert_eq!(
        initial_set, expected_layer_0,
        "only layer 0 should be initially ready"
    );
