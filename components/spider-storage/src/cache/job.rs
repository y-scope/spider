...
> SharedJobControlBlock<ReadyQueueSenderType, DbConnectorType, TaskInstancePoolConnectorType>
{
    ...
    pub async fn start(&self) -> Result<(), CacheError> {
        ...
        job.ready_queue_sender
            .send_task_ready(jcb.id, jcb.owner_id, ready_task_indices)
            .await?;
        drop(job);
        Ok(())
    }
    ...
    pub async fn succeed_task_instance(
        ...
        if has_commit_task {
            job.ready_queue_sender
                .send_commit_ready(jcb.id, jcb.owner_id)
                .await?;
        }
        Ok(job.state)
    }
        ...
        match task_id {
            TaskId::Index(task_index) => {
                ...
                if matches!(task_state, TaskState::Ready | TaskState::Running) {
                    job.ready_queue_sender
                        .send_task_ready(jcb.id, jcb.owner_id, vec![task_index])
                        .await?;
                    return Ok(job.state);
                }
            }
            ...
            TaskId::Cleanup => {
                ...
                if matches!(task_state, TaskState::Ready | TaskState::Running) {
                    job.ready_queue_sender
                        .send_cleanup_ready(jcb.id, jcb.owner_id)
                        .await?;
                    return Ok(job.state);
                }
            }
